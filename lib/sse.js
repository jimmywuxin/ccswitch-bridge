...
export class SseTranslator {
    ...
    if (delta.content) {
      if (this._inlineThinkEnabled) {
        ...
        if (!this._thinkClosed) {
          ...
          // Think already closed; emit content as normal text
  ...
  _feedReasoning(text) {
    ...
    if (!this.reasoningStarted) {
      this.reasoningStarted = true;
      this.reasoningItemId = "rsn_" + randomUUID().replace(/-/g, "").slice(0, 8);
      const oi = this.outputItemCount++;
  ...
  _feedReasoningItem(text, completed = false) {
    ...
    this._ensureStarted();
    const id = "rsn_" + randomUUID().replace(/-/g, "").slice(0, 8);
    const oi = this.outputItemCount++;
  ...
  _feedTextDelta(text) {
    ...
    // Flush any remaining inline think buffer as text (no closing <think> tag was received)
    if (this._inlineThinkEnabled && !this._thinkClosed && this._thinkBuf) {
      this._thinkClosed = true;
      this.contentSoFar += this._thinkBuf;
      this._feedTextDelta(this._thinkBuf);
      this._thinkBuf = "";
